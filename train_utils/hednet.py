...
class HEDnet:
    ...
    def build(self, rgb, scope="HEDnet", reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            _, self.conv2_1 = self.conv_layer(self.pool1, "conv2_1")
            _, self.conv2_2 = self.conv_layer(self.conv2_1, "conv2_2")
            b2 = self.branch('branch2', self.conv2_2, 2)
            self.pool2 = self.max_pool(self.conv2_2, 'pool2')
            
            _, self.conv3_1 = self.conv_layer(self.pool2, "conv3_1")
            _, self.conv3_2 = self.conv_layer(self.conv3_1, "conv3_2")
            _, self.conv3_3 = self.conv_layer(self.conv3_2, "conv3_3")
            b3 = self.branch('branch3', self.conv3_3, 4)
            self.pool3 = self.max_pool(self.conv3_3, 'pool3')
            
            _, self.conv4_1 = self.conv_layer(self.pool3, "conv4_1")
            _, self.conv4_2 = self.conv_layer(self.conv4_1, "conv4_2")
            _, self.conv4_3 = self.conv_layer(self.conv4_2, "conv4_3")
            b4 = self.branch('branch4', self.conv4_3, 8)
            self.pool4 = self.max_pool(self.conv4_3, 'pool4')
            
            _, self.conv5_1 = self.conv_layer(self.pool4, "conv5_1")
            _, self.conv5_2 = self.conv_layer(self.conv5_1, "conv5_2")
            _, self.conv5_3 = self.conv_layer(self.conv5_2, "conv5_3")
